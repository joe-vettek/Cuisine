    ...
    public static class Overrides extends ItemOverrides {
        ...
        public static Map<String, BlockDefinition> overridesFromItem(ItemStack stack) {
            ...
            try {
                ...
                data= ((CompoundTag)stack.getOrCreateTag().get("BlockEntityTag")).getCompound("Overrides");
            }catch (Exception ex){
                ex.printStackTrace();
//                Cuisine.logger("overridesFromItem",stack.getOrCreateTag());
            }
            if (data == null)
            ...
            readTextures(overrides, data, Predicates.alwaysTrue());
            return overrides;
        }
    ...
    public static boolean readTextures(Map<String, BlockDefinition> textures, CompoundTag helper, Predicate<BlockDefinition> validator) {
        if (textures == null) {
            return false;
        }
...
//		NBTHelper helper = NBTHelper.of(data);
        try {
            for (String k : textures.keySet()) {
                CompoundTag v = (CompoundTag) helper.get(k);
                if (v == null)
                    continue;
